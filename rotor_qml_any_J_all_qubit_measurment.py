...
def _circuit(x, params):
    # qml.broadcast(qml.PauliX, wires=range(num_qubits), pattern="single")
    # qml.QubitUnitary(get_R(x), wires=range(num_qubits))
    ...
    '''qml.broadcast(
        qml.U3, wires=range(num_qubits), pattern="single", parameters=params[-1]
    )'''

    return [qml.expval(qml.PauliZ(i)) for i in dev.wires]


circuit = jax.jit(_circuit)

weights = np.random.uniform(-jnp.pi, jnp.pi, [num_layers, num_qubits, 3])
weights = jnp.array(weights)

'''fig, ax = qml.draw_mpl(_circuit)(X_train[0], weights)
fig.show()'''
...
def predict_one(x, parameters):
    ...
    # majority = jnp.sum(2 * (ep >= 0) - 1)
    majority = jnp.mean(ep)
    return 2 * (majority >= 0) - 1
...
resources = qml.specs(_circuit)(X_train[0], weights)
print(resources)
